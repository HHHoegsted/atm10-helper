...
def import_language(
    ...
    for language_file in language_files:
        language_entries.update(_parse_language_entries(language_file))

    chapter_updates = _build_chapter_language_updates(language_entries)
    quest_updates = _build_quest_language_updates(language_entries)
        ...
        if depth == 1:
            ...
            if match is not None:
                ...
                if raw_value.startswith('"'):
                    entries[key] = _clean_minecraft_text(_parse_snbt_string(raw_value))
                ...
                if raw_value.startswith("["):
                    ...
                    value_depth = _value_depth(raw_value)
                    ...
                    while value_depth > 0:
                        ...
                        value_depth += _value_depth(lines[index])

                    entries[key] = [
                        _clean_minecraft_text(value)
                        for value in _parse_snbt_string_list("\n".join(collected_lines))
                    ]
                    depth = _updated_depth(depth, line)
                    index += 1
                    continue
...
def _build_chapter_language_updates(
    language_entries: dict[str, str | list[str]],
    ...
    for key, value in language_entries.items():
        ...
        if field == "title":
            update["title"] = _language_value_to_text(value)

        if field == "chapter_subtitle":
            update["subtitle"] = _language_value_to_text(value)

    return updates


def _build_quest_language_updates(
    language_entries: dict[str, str | list[str]],
) -> dict[str, dict[str, str]]:
    updates: dict[str, dict[str, str]] = {}
    ...
    for key, value in language_entries.items():
        ...
        if field == "quest_desc":
            update["description"] = _language_value_to_text(value)

    return updates


def _language_value_to_text(value: str | list[str]) -> str | None:
...
def _parse_snbt_string(raw_value: str) -> str:
    match = re.match(r'^"((?:\\.|[^"\\])*)"', raw_value.strip())

    if match is None:
        raise ValueError(f"Could not parse SNBT string value: {raw_value}")

    return _decode_snbt_string(match.group(1))


def _parse_snbt_string_list(raw_value: str) -> list[str]:
    return [_decode_snbt_string(match) for match in re.findall(r'"((?:\\.|[^"\\])*)"', raw_value)]
...
def _clean_minecraft_text(value: str) -> str:
...
def _value_depth(value: str) -> int:
    depth = 0
    in_string = False
    escaped = False
